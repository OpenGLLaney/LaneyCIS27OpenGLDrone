...
int main()
{
  ...
  // drone
  Shader selfDefinedShader("resources/shaders/shader.vs", "resources/shaders/shader.fs");
  //tree
  Shader modelShader("resources/shaders/modelShader.vs", "resources/shaders/modelShader.fs");
  // other things like trees/house/rocks etc
  
  // load models
  // -----------
  Model droneModel("resources/models/drone1/Drone.obj");

  Model tree("resources/models/tree1/tree.3ds");
...
//  shader.setInt("texture1", 0);
  ...
  selfDefinedShader.setInt("floor", 0);
  
  modelShader.use();
  modelShader.setInt("model", 0);
  // render loop
  // -----------
  while (!glfwWindowShouldClose(window))
  {
    ...
    glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 1400.0f);
    //floor
    glBindVertexArray(floorVAO);
    ...
    modelShader.setMat4("view", droneview);
    droneModel.Draw(modelShader);
...
void processInput(GLFWwindow *window)
{
  ...
  //drone move and camera move together
